...
const initialDestinaionsState = {
  ...
  wishList: [],
  searchTerm: "",
  filteredDistinations: destinations,
};
...
export const sendWishDestination = (destinations) => {
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      for (const key in destinationsData) {
        wishListData.push(destinationsData[key]);
      }
      dispatch(Actions.setWishList(wishListData));
    } catch (error) {
      console.log(error);
    }
  };
